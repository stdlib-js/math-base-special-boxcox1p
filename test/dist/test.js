...
var tinyLargeLambda = require( './../..xtures/python/tiny_and_large_lambda.json' );
...
tape( 'the function returns `-Infinity` if lambda is negative and `x` is negative one', function test( t ) {
	var lambda;
	var b;
	var i;
	...
	for ( i = 0; i < 1e3; i++ ) {
		...
		t.equal( b, NINF, 'returns -Infinity when lambda is negative and `x` is negative one' );
	}
	t.end();
});

tape( 'the function returns `NaN` if `x` is less than negative one', function test( t ) {
	var b;
	var x;
	var y;
	var i;
